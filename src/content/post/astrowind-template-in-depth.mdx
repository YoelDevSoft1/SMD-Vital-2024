...
It can be a somewhat daunting task trying to get a handle on _SMD Vital Bogota_ internals, and particularly various points of usage.

This page outlines and clarifies some of the techniques found in _SMD Vital Bogota_. Use it as a guide for further modification, or an instructional for techniques to use in your own endeavors.

## Styling

As the name suggests, _SMD Vital Bogota_ relies on _TailWind_ for styling. Furthermore, _SMD Vital Bogota_ defines custom low level style settings which are incorporated into _TailWind_ seamlessly, and which provides consistency for higher level styling constructs, as well as enabling dark mode.
...
<DListItem dt="layouts/Layout.astro">
  This layout is used for all of the pages rendered by _SMD Vital Bogota_. The contents of _tailwind.css_ and
...
Now, we get to the techniques used in _SMD Vital Bogota_, we'll use the _pages/index.astro_ file to illustrate.
...
<CallToAction
  callToAction={{
    text: 'Get template',
    href: 'https://github.com/Yoel Dev Soft/astrowind',
